...
// ===== Cuando todo el DOM y las librerías están listas =====
window.addEventListener("DOMContentLoaded", () => {
  ...
  const updatePrices = (yearly) => {
    ...
    period.forEach(
      (p) => (p.textContent = yearly ? "/mes (anual)" : "/mes")
    );
  };
  ...
  if (window.gsap && window.ScrollTrigger && window.TextPlugin) {
    ...
    $$(".section").forEach((sec) => {
      gsap.from(
        sec.querySelectorAll(
          ".section-title, .section-subtitle, .card, .case, .review, .phone, .demo-tabs, .demo-panels"
        ),
        {
          opacity: 0,
          y: 26,
    ...
    if (wrap) {
      wrap.addEventListener("mousemove", (e) => {
        ...
        gsap.to(".glow-a", { x: x * 20, y: y * 20, duration: 0.3 });
        gsap.to(".glow-b", { x: -x * 20, y: -y * 20, duration: 0.3 });
      });
    }
  }
  ...
  form?.addEventListener("submit", async (e) => {
    ...
    try {
      if (
        ...
        throw new Error("Faltan keys EmailJS");
    ...
    try {
      ...
      formMsg.textContent =
        "Abrimos tu cliente de correo para completar el envío ✉️";
    } catch (e) {
      formMsg.textContent =
        "No pudimos enviar el mensaje. Escribinos por WhatsApp, por favor.";
    }
  });
});
